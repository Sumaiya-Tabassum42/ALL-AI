...
const statusStagesByService: Record<string, string[]> = {
  text: ["Thinking...", "Generating response..."],
  image: ["Thinking...", "Generating image...", "Almost done..."],
  document: ["Thinking...", "Writing content...", "Formatting document...", "Almost done..."],
  presentation: [
    "Thinking...",
    "Planning your slide outline...",
    "Writing slide content...",
    "Designing your slides...",
    "Almost done, exporting your presentation...",
  ],
  data_analysis: ["Thinking...", "Analyzing your data...", "Fetching insights...", "Almost done..."],
  default: ["Thinking...", "Generating response..."],
}
...
async function handleSend() {
  ...
  try {
    ...
    console.log("DATASET BEFORE SEND:", datasetRef.current)

    // Start cycling status messages for every service
    const stages = statusStagesByService[service] || statusStagesByService.default
    let stageIndex = 0
    setGenerationStatus(stages[0])

    statusInterval = setInterval(() => {
      stageIndex = (stageIndex + 1) % stages.length
      setGenerationStatus(stages[stageIndex])
    }, 4000)
    ...
    const response = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        user_id: user.id,
        prompt: message,
        dataset: service === "data_analysis" ? datasetRef.current : []
      }),
    })
      ...
      <div className="mx-auto max-w-4xl">
...

        {
          sending && generationStatus && (
            <div className="mb-3 flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600 italic animate-pulse">
